...
def generate_image(user_description):
    prompt = f"You are a professional psychological counseler. Based on the below user's description and content, create a proper a psychotherapy session: {user_description}"
...
def understand_image(base64_image):
    prompt = "Analyze the content of this image and write a proper a psychochological therapy` session:"
...
def main():
    st.set_page_config(page_title="PSYCHGPT", layout="wide")
    st.title("PSYCHGPT")

    with st.sidebar:
        st.header("Controls")
        image_description = st.text_area("Description for Psychatric Session Generation", height=100)
        generate_image_btn = st.button("Generate Session")

    col1, col2 = st.columns(2)

    with col1:
        st.header("Psych Scenario")
        if generate_image_btn and image_description:
            with st.spinner("Generating image..."):
                ...
                if image_path:
                    st.image(
                        image_path,
                        caption="Generated Psychatric Session",
    ...
    with col2:
        ...
        if generate_image_btn and image_description:
            with st.spinner("Creating recording..."):
